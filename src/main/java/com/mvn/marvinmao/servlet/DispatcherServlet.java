package com.mvn.marvinmao.servlet;
...
import com.mvn.marvinmao.controller.MarvinMaoController;
import com.mvn.marvinmao.handlerAdapter.HandlerAdapterService;
...
public class DispatcherServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        //获取到请求路径   /marvinmao-springmvc/james/query
        String uri = request.getRequestURI();

        String context = request.getContextPath();
        //将  "/marvinmao-springmvc/james/query"  去掉"/marvinmao-springmvc"
        String path = uri.replace(context, "");
        //根据请求路径来获取要执行的方法
        Method method = (Method) handlerMap.get(path);
        //拿到控制类
        MarvinMaoController instance = (MarvinMaoController) beans.get("/" + path.split("/")[1]);
        //处理器
        ...
        try {
			...
			*用策略模式实现(把粒度控制得更细),新建 MarvinMaoHandlerAdapter
			*/
    ...
    private void HandlerMapping() {
        ...
        for (Map.Entry<String, Object> entry : beans.entrySet()) {
            ...
            // 拿所有Controller的类
            if (clazz.isAnnotationPresent(MarvinController.class)) {
                //@MarvinRequestMapping(value=/james)
                MarvinRequestMapping requestMapping = (MarvinRequestMapping) clazz
                        .getAnnotation(MarvinRequestMapping.class);
                ...
                for (Method method : methods) {
                    ...
                    if (method.isAnnotationPresent(MarvinRequestMapping.class)) {
                        //@MarvinRequestMapping(value=/query)
                        MarvinRequestMapping methodrm = (MarvinRequestMapping) method
                                .getAnnotation(MarvinRequestMapping.class);
                        String methodPath = methodrm.value();
                        // 把方法上与路径建立映射关系( /james/query--->public void MarvinMaoController.query )
        ...
        for (Map.Entry<String, Object> entry : beans.entrySet()) {
            ...
            if (clazz.isAnnotationPresent(MarvinController.class)) {
                ...
                for (Field field : fields) {
                    if (field.isAnnotationPresent(MarvinQualifier.class)) {
                        MarvinQualifier qualifier = (MarvinQualifier) field.getAnnotation(MarvinQualifier.class);
                        //拿到@MarvinQualifier("MarvinMaoServiceImpl")里的指定要注入的bean名字"MarvinMaoServiceImpl"
                        String value = qualifier.value();

                        field.setAccessible(true);
                        try {
                            // 从MAP容器中获取"MarvinMaoServiceImpl"对应的bean,并注入实例控制层bean,解决依赖注入
    ...
    private void instance() {
        ...
        for (String className : classNames) {
            ...
            try {
                ...
                if (clazz.isAnnotationPresent(MarvinController.class)) {
                    MarvinController controller = (MarvinController) clazz.getAnnotation(MarvinController.class);
                ...
                } else if (clazz.isAnnotationPresent(MarvinService.class)) {
                    //获取当前clazz类的注解(通过这个注解可得到当前service的id)  @MarvinService(value=MarvinMaoServiceImpl)
                    MarvinService service = (MarvinService) clazz.getAnnotation(MarvinService.class);
                    Object instance = clazz.newInstance();
                    //put(MarvinMaoServiceImpl,instance)
